...
В данном конкретном наборе многие колонки заполнены нулями. Для более точной информации следует смотреть `03_Day/sensor_data/inspva.csv` - там есть исчерпывающие INS/IMU-значения (roll/pitch/azimuth и компоненты скорости).
...
Ближайший по временной метке кадр камеры автоматически подбирается и отображается на графике.

Результат — графики matplotlib:
- Radial velocity vs azimuth (если есть Допплер) - серые точки = static, красные = moving, чёрная линия = RANSAC-кривая эго-движения
- Bird's-eye view (x, y) - те же цвета
- При `--camera`: изображение со стерео-камеры
...
| estimator       | reference |    N |   TP |  FP |  FN | TN |   Acc |  Prec |   Rec |    F1 |
|-----------------|-----------|-----:|-----:|----:|----:|---:|------:|------:|------:|------:|
| Aeva (RANSAC)   | ИНС       | 1500 | 1500 |   0 |   0 |  0 | 1.000 | 1.000 | 1.000 | 1.000 |
| Aeva (RANSAC)   | GPS       | 1500 | 1425 |  75 |   0 |  0 | 0.950 | 0.950 | 1.000 | 0.974 |
| Radar (RANSAC)  | ИНС       | 3000 | 3000 |   0 |   0 |  0 | 1.000 | 1.000 | 1.000 | 1.000 |
| Radar (RANSAC)  | GPS       | 3000 | 2846 | 154 |   0 |  0 | 0.949 | 0.949 | 1.000 | 0.974 |
| GPS             | ИНС       | 7418 | 7093 |   0 | 325 |  0 | 0.956 | 1.000 | 0.956 | 0.978 |
...
- **Acc/Prec/Rec/F1** показывают, насколько надёжно сенсор детектирует факт движения. На рассматриваемом отрезке аппарат почти всё время в движении, поэтому отрицательный класс («стоит») малочислен.
- **MAE/RMSE/bias** — точность самой величины |V|. Relative-ground-truth — ИНС (гладкая высокочастотная оценка); GPS-скорость из конечных разностей шумная и содержит выбросы, поэтому RMSE и max|err| относительно GPS высокие у всех - это шум reference, а не ошибка оценщика.
- **bias > 0** у Aeva и Radar - обе RANSAC-оценки систематически завышают |V| относительно ИНС (Aeva на ~0.22 м/с, Radar на ~0.03 м/с).
