...
**Как читать:**
- **Acc/Prec/Rec/F1** показывают, насколько надёжно сенсор детектирует факт движения. На рассматриваемом отрезке аппарат почти всё время в движении, поэтому отрицательный класс («стоит») малочислен — отсюда `Spec = n/a` (нет TN+FP) либо нули.
- **MAE/RMSE/bias** — точность самой величины |V|. Relative-ground-truth — ИНС (гладкая высокочастотная оценка); GPS-скорость из конечных разностей шумная и содержит выбросы, поэтому RMSE и max|err| относительно GPS высокие у всех — это шум reference, а не ошибка оценщика. Сравнение GPS vs ИНС в последней строке служит sanity-check масштаба этого шума.
